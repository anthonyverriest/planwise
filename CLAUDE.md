...
- Verify before claiming: never say "should work" or "seems to pass" — run the command, read the output, cite the evidence. For bug fixes, demonstrate the test fails without the fix and passes with it.
- When acting on review feedback, verify the suggestion is correct for this codebase before applying. Grep for actual usage before implementing "proper" patterns a reviewer requests.
</approach>
...
- Never test mock behavior instead of real behavior; mocks must reflect actual API responses including all fields downstream code uses.

...
```bash
planwise view <ARGS> --field type
planwise view <ARGS> --field status
```

**Guards:**
- Type must be `feature`. If not → stop: `<ARGS> is not a feature.`
- Status must be `done` or `in-review`. If `in-review`, warn: `Feature <ARGS> is in-review, not done. Proceeding — knowledge may be incomplete.` Any other status → stop: `Feature <ARGS> is not complete (status: <status>). Finish the feature first.`
...
```bash
planwise list --children-of <ARGS> --type sub-feature
planwise list --children-of <ARGS> --type bug
...
planwise list --children-of <ARGS> --type sub-feature
planwise list --children-of <ARGS> --type bug
planwise list --children-of <ARGS> --type uat
...
```bash
jj log -r 'description(glob:"*'"<ARGS>"'*") & ::dev@origin & merges()' -T builtin_log_oneline --stat
```
...
### Dispatch — distillation subagents (general-purpose)
...
### Step 2: Dispatch — lessons distiller (general-purpose)
...
```
Knowledge base updated for Feature <ARGS>.

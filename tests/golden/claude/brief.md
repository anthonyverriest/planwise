...
- **Issue reference** — `<ARGS>` matches an existing issue slug (e.g., `user-auth`, `login-endpoint`).
...
Build the task body through conversation — this IS the plan. Focus on the *what* and *why*. Work through each section with the user — ask focused questions to fill gaps, don't move on until each section is clear.
...
Use `AskUserQuestion` tool: **"Task spec ready. Create the issue, or stop here?"**

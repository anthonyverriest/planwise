...
# Implement Feature <the user's task>

Implement every coding sub-feature of <the user's task> in dependency order.
...
Loop until `planwise ready --children-of <the user's task>` returns nothing. Each iteration:

### 3a. Fetch the ready batch

```bash
planwise ready --children-of <the user's task>
```
...
Use a worktree-isolated subagent for every sub-feature, even when the batch has only one. Uniform dispatch removes the serial/parallel fork. The subagent spawns the worktree from the current workspace's HEAD, so sub-features branch off the latest feature tip automatically.
...
- **N > 1:** n-way merge the parallel branches onto the current feature tip.
  ```bash
  jj new @ sub/<slug-a>@git sub/<slug-b>@git ... -m "merge: <slugs> (#<the user's task>)"
  ```
...
Clean up every consumed `sub/<slug>` git ref — both the N==1 and N>1 paths — so batches don't accumulate stale branches (the subagent runner reclaims the worktree itself, but leaves the ref):
...
Re-run `planwise ready --children-of <the user's task>`. Repeat until empty.
...
3. **Dispatch — Cross-boundary reviewer (fresh context)**
...
4. **If findings returned,** fix and commit:
   ```bash
   jj commit -m "ref: <specific fix> (#<the user's task>)"
   ```
...
### Dispatch — Tour writer (planwise-worker, fresh context)
...
```markdown
Tour unavailable — see plan: `planwise view <the user's task>`
...
```bash
planwise list --children-of <the user's task> --type uat | jq -r '.[0].id'
...
```bash
planwise status <the user's task> in-review
```
...
```
Feature <the user's task> complete.

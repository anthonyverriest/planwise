...
- **Standalone bug** — first token is not a known slug. Whole `<the user's task>` is the description. No parent.
...
Use `AskQuestion` if available (fallback: ask in chat): **"Bug spec ready. Create the issue, or stop here?"**
